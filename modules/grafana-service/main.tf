...
module "grafana-rds" {
  ...
  username          = "${data.aws_ssm_parameter.grafana-rds-username.value}"
  password          = "${data.aws_ssm_parameter.grafana-rds-password.value}"
...
module "grafana-service" {
  ...
  task_container_image             = "teliaoss/grafana-aws-env:v5.2.4"
  ...
  task_container_environment = {
    ...
    "GF_SERVER_ROOT_URL"                   = "ssm://${aws_ssm_parameter.grafana-root-url.name}"
    "GF_DATABASE_TYPE"                     = "postgres"
    "GF_DATABASE_USER"                     = "ssm://${data.aws_ssm_parameter.grafana-rds-username.name}"
    "GF_DATABASE_PASSWORD"                 = "ssm://${data.aws_ssm_parameter.grafana-rds-password.name}"
    "GF_DATABASE_HOST"                     = "ssm://${aws_ssm_parameter.grafana-rds-host.name}"
    "GF_SECURITY_ADMIN_USER"               = "ssm://${data.aws_ssm_parameter.grafana-admin-user-name.name}"
    "GF_SECURITY_ADMIN_PASSWORD"           = "ssm://${data.aws_ssm_parameter.grafana-admin-user-password.name}"
    "GF_AUTH_GITHUB_ENABLED"               = "ssm://${data.aws_ssm_parameter.grafana-github-auth-enabled.name}"
    "GF_AUTH_GITHUB_CLIENT_ID"             = "ssm://${data.aws_ssm_parameter.grafana-github-client-id.name}"
    "GF_AUTH_GITHUB_CLIENT_SECRET"         = "ssm://${data.aws_ssm_parameter.grafana-github-client-secret.name}"
    "GF_AUTH_GITHUB_ALLOWED_ORGANISATIONS" = "ssm://${data.aws_ssm_parameter.grafana-github-allowed-organisations.name}"
...
# ----------------------------------------
data "aws_ssm_parameter" "grafana-rds-username" {
  name = "/${var.name_prefix}/rds-username"
}

data "aws_ssm_parameter" "grafana-rds-password" {
  name = "/${var.name_prefix}/rds-password"
}

resource "aws_ssm_parameter" "grafana-root-url" {
...
resource "aws_ssm_parameter" "grafana-rds-host" {
...
data "aws_ssm_parameter" "grafana-admin-user-name" {
  name = "/${var.name_prefix}/admin-user-name"
}

data "aws_ssm_parameter" "grafana-admin-user-password" {
  name = "/${var.name_prefix}/admin-user-password"
}

data "aws_ssm_parameter" "grafana-github-auth-enabled" {
  name = "/${var.name_prefix}/github-auth-enabled"
}

data "aws_ssm_parameter" "grafana-github-client-id" {
  name = "/${var.name_prefix}/github-client-id"
}

data "aws_ssm_parameter" "grafana-github-client-secret" {
  name = "/${var.name_prefix}/github-client-secret"
}

data "aws_ssm_parameter" "grafana-github-allowed-organisations" {
